...
import { ArrowLeft, CheckCircle, ChevronLeft, ChevronRight, CreditCard, Loader2, RefreshCw, WalletCards, XCircle } from 'lucide-react';
...
export default function SettlementsDashboard() {
  ...
  const [pagination, setPagination] = useState({ page: 1, page_size: SETTLEMENTS_PAGE_SIZE, total: 0, total_pages: 0, has_next: false, has_previous: false });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [pendingResponse, settlementsResponse] = await Promise.all([
        ...
        settlementAPI.getAll({ ...orgParams, page: currentPage, page_size: SETTLEMENTS_PAGE_SIZE })
      ]);
      setPending(pendingResponse.data || []);
      const page = settlementsResponse.data || {};
      setSettlements(page.items || []);
      setPagination({ page: page.page || currentPage, page_size: page.page_size || SETTLEMENTS_PAGE_SIZE, total: page.total || 0, total_pages: page.total_pages || 0, has_next: Boolean(page.has_next), has_previous: Boolean(page.has_previous) });
    } catch (error) {
      toast.error(error.response?.data?.detail || 'No fue posible cargar las liquidaciones');
    } finally { setLoading(false); }
  }, [organizationId, period.period_start, period.period_end, currentPage]);

  useEffect(() => { setCurrentPage(1); }, [organizationId, period.period_start, period.period_end]);
  useEffect(() => { load(); }, [load]);
  ...
  return <div className="min-h-screen nexus-screen text-[var(--app-text-primary)]">
    ...
    <main className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      {loading ? <div className="py-20 flex justify-center"><Loader2 className="animate-spin text-[var(--app-primary)]" size={36}/></div> : <>
        <section className="rounded-2xl nexus-panel border border-[var(--app-border)] overflow-hidden"><div className="p-5 border-b border-[var(--app-border)]"><h2 className="text-lg">Pendiente por liquidar</h2><p className="text-sm text-[var(--app-text-muted)]">{pending.length} profesionales con movimientos disponibles</p></div>{pending.length ? pending.map(row => <div key={row.barber_id} className="p-4 border-b border-[var(--app-border)] flex flex-col md:flex-row md:items-center justify-between gap-4"><div><p className="font-medium">{row.staff_name}</p><p className="text-sm text-[var(--app-text-secondary)]">{row.transaction_count} transacciones · Comisión {money(row.commission_amount)} · Propinas {money(row.tip_amount)}</p></div><div className="flex items-center gap-3"><b className="text-emerald-400 text-lg">{money(row.total_amount)}</b><button disabled={busy} onClick={() => createSettlement(row)} className="px-4 py-2 bg-[var(--app-primary)] rounded-xl disabled:opacity-50">Crear borrador</button></div></div>) : <p className="p-12 text-center text-[var(--app-text-muted)]">No hay transacciones pendientes para este periodo.</p>}</section>
        <section className="rounded-2xl nexus-panel border border-[var(--app-border)] overflow-hidden"><div className="p-5 border-b border-[var(--app-border)]"><h2 className="text-lg">Historial de liquidaciones</h2><p className="text-sm text-[var(--app-text-muted)]">{pagination.total} registros</p></div>{settlements.length ? settlements.map(item => <div key={item.settlement_id} className="p-4 border-b border-[var(--app-border)] flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div><div className="flex flex-wrap gap-2 items-center"><p className="font-medium">{item.staff_name_snapshot}</p><span className={`px-2 py-1 rounded-lg border text-xs ${STATUS_CLASS[item.status]}`}>{STATUS[item.status]}</span></div><p className="text-sm text-[var(--app-text-secondary)] mt-1">{item.period_start} a {item.period_end} · {item.transaction_count} transacciones · {item.settlement_id}</p></div><div className="flex flex-wrap items-center gap-2"><b className="text-lg mr-2">{money(item.total_amount)}</b><button onClick={() => details(item)} className="px-3 py-2 nexus-panel border border-[var(--app-border)] rounded-xl">Detalle</button>{item.status === 'draft' && <button onClick={() => approve(item)} className="px-3 py-2 bg-blue-500/20 text-blue-300 rounded-xl">Aprobar</button>}{item.status === 'approved' && <button onClick={() => pay(item)} className="px-3 py-2 bg-emerald-500/20 text-emerald-300 rounded-xl">Registrar pago</button>}{['draft','approved'].includes(item.status) && <button onClick={() => cancel(item)} className="px-3 py-2 bg-red-500/10 text-red-300 rounded-xl">Cancelar</button>}</div></div>) : <p className="p-12 text-center text-[var(--app-text-muted)]">Aún no hay liquidaciones.</p>}{pagination.total_pages > 1 && <div className="p-4 border-t border-[var(--app-border)] flex flex-col sm:flex-row items-center justify-between gap-3"><span className="text-sm text-[var(--app-text-muted)]">Mostrando {(pagination.page - 1) * pagination.page_size + 1} - {Math.min(pagination.page * pagination.page_size, pagination.total)} de {pagination.total}</span><div className="flex items-center gap-3"><button type="button" aria-label="Página anterior" disabled={!pagination.has_previous || loading} onClick={() => setCurrentPage(page => Math.max(1, page - 1))} className="p-2 nexus-panel border border-[var(--app-border)] rounded-xl disabled:opacity-40"><ChevronLeft size={18}/></button><span className="text-sm text-[var(--app-text-muted)]">Página {pagination.page} de {pagination.total_pages}</span><button type="button" aria-label="Página siguiente" disabled={!pagination.has_next || loading} onClick={() => setCurrentPage(page => Math.min(pagination.total_pages, page + 1))} className="p-2 nexus-panel border border-[var(--app-border)] rounded-xl disabled:opacity-40"><ChevronRight size={18}/></button></div></div>}</section>
      </>}
    </main>
    {selected && <AccessibleModal open={!!selected} onClose={()=>setSelected(null)} labelledBy="settlement-detail-title" panelClassName="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-[var(--app-surface-elevated)] border border-[var(--app-border)] rounded-2xl p-6"><button onClick={() => setSelected(null)} className="float-right">Cerrar</button><h2 id="settlement-detail-title" className="text-xl mb-4">Detalle de liquidación</h2><div className="grid grid-cols-2 gap-2 text-sm mb-5">{[['Profesional',selected.staff_name_snapshot],['Estado',STATUS[selected.status]],['Comisiones',money(selected.commission_amount)],['Propinas',money(selected.tip_amount)],['Total',money(selected.total_amount)],['Referencia',selected.payment_reference || 'Sin referencia']].map(([a,b]) => <React.Fragment key={a}><span className="text-[var(--app-text-muted)]">{a}</span><span className="text-right">{b}</span></React.Fragment>)}</div><h3 className="font-medium mb-2">Transacciones incluidas</h3>{(selected.transactions||[]).map(tx => <div key={tx.transaction_id} className="py-3 border-t border-[var(--app-border)] flex justify-between gap-3"><span>{tx.service_name_snapshot} · {tx.transaction_id}</span><span>{money(tx.staff_total_amount)}</span></div>)}</AccessibleModal>}
    {workflow && <AccessibleModal open={!!workflow} onClose={()=>!busy&&setWorkflow(null)} labelledBy="settlement-workflow-title" describedBy="settlement-workflow-description" className="nexus-confirm-layer" panelClassName="nexus-accessible-modal-panel"><h2 id="settlement-workflow-title">{workflow.type === 'pay' ? 'Registrar pago' : 'Cancelar liquidación'}</h2><p id="settlement-workflow-description">{workflow.item.staff_name_snapshot} · {money(workflow.item.total_amount)}</p>{workflow.type === 'pay' ? <><label>Método<select className="nexus-field" value={form.payment_method} onChange={e => setForm({...form,payment_method:e.target.value})}><option value="transfer">Transferencia</option><option value="cash">Efectivo</option><option value="card">Tarjeta</option><option value="other">Otro</option></select></label><label>Referencia<input className="nexus-field" value={form.payment_reference} onChange={e => setForm({...form,payment_reference:e.target.value})}/></label></> : <label>Motivo<textarea className="nexus-field" value={form.reason} onChange={e => setForm({...form,reason:e.target.value})}/></label>}<div className="nexus-account-actions"><button className="nexus-action-button nexus-action-secondary" onClick={() => setWorkflow(null)}>Cerrar</button><button className={`nexus-action-button ${workflow.type==='cancel'?'nexus-action-destructive':'nexus-action-primary'}`} onClick={submitWorkflow}>Confirmar</button></div></AccessibleModal>}
  </div>;
}