...
const StaffProfile = () => {
  ...
  return (
    <div className="min-h-screen nexus-screen text-[var(--app-text-primary)] p-4 sm:p-6">
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-wrap items-center gap-3 mb-8">
          <button type="button" onClick={() => window.history.back()} className="p-2 rounded-xl nexus-panel border border-[var(--app-border)] hover:bg-white/10"><ArrowLeft size={20} /></button>
          <div className="mr-auto">
            <h1 className="text-3xl sm:text-4xl font-light">Mi perfil profesional</h1>
            <p className="text-[var(--app-text-secondary)] text-sm mt-1">Actualiza cómo apareces ante los clientes.</p>
          </div>
        ...
        <div className="grid lg:grid-cols-[1.5fr_1fr] gap-6">
          <div className="space-y-6 rounded-2xl border border-[var(--app-border)] nexus-panel p-5 sm:p-7">
            <section>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <label className="text-sm text-[var(--app-text-secondary)]">Nombre<input value={profile.first_name} onChange={(event) => setField('first_name', event.target.value)} className={`${inputClass} mt-2`} /></label>
                <label className="text-sm text-[var(--app-text-secondary)]">Apellido<input value={profile.last_name} onChange={(event) => setField('last_name', event.target.value)} className={`${inputClass} mt-2`} /></label>
                <label className="text-sm text-[var(--app-text-secondary)] sm:col-span-2">Nombre visible<input value={profile.display_name} onChange={(event) => setField('display_name', event.target.value)} className={`${inputClass} mt-2`} /></label>
                <label className="text-sm text-[var(--app-text-secondary)]">Teléfono<input value={profile.phone} onChange={(event) => setField('phone', event.target.value)} className={`${inputClass} mt-2`} /></label>
                <label className="text-sm text-[var(--app-text-secondary)]">Dirección<input value={profile.address} onChange={(event) => setField('address', event.target.value)} className={`${inputClass} mt-2`} /></label>
                <label className="text-sm text-[var(--app-text-secondary)] sm:col-span-2">URL de avatar<input value={profile.avatar} onChange={(event) => setField('avatar', event.target.value)} className={`${inputClass} mt-2`} placeholder="https://..." /></label>
                <label className="text-sm text-[var(--app-text-secondary)] sm:col-span-2">Biografía <span className="float-right">{profile.bio.length}/500</span><textarea value={profile.bio} onChange={(event) => setField('bio', event.target.value.slice(0, 500))} className={`${inputClass} mt-2 min-h-[110px] resize-y`} /></label>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {DAYS.map((day) => <button key={day.value} type="button" onClick={() => toggleDay(day.value)} className={`px-3 py-2 rounded-lg border text-sm ${(profile.available_days || []).includes(day.value) ? 'bg-[#0A84FF]/20 border-[#0A84FF]/50 text-[#5EB1FF]' : 'nexus-panel border-[var(--app-border)] text-[var(--app-text-secondary)]'}`}>{day.label}</button>)}
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <label className="text-sm text-[var(--app-text-secondary)]">Hora inicio<input type="time" value={profile.start_time} onChange={(event) => setField('start_time', event.target.value)} className={`${inputClass} mt-2`} /></label>
                <label className="text-sm text-[var(--app-text-secondary)]">Hora fin<input type="time" value={profile.end_time} onChange={(event) => setField('end_time', event.target.value)} className={`${inputClass} mt-2`} /></label>
              </div>
            </section>
            ...
            <section>
              ...
              <p className="text-xs text-[var(--app-text-muted)] mb-3">La administración de la barbería asigna estos servicios.</p>
              <div className="flex flex-wrap gap-2">{assignedServices.length ? assignedServices.map((service) => <span key={service.service_id} className="px-3 py-2 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-300 text-sm">{service.name}</span>) : <span className="text-sm text-[var(--app-text-muted)]">No hay servicios asignados.</span>}</div>
            </section>
          ...
          <aside className="rounded-2xl border border-[var(--app-border)] nexus-panel p-6 h-fit lg:sticky lg:top-6">
            <p className="text-xs uppercase tracking-wider text-[var(--app-text-muted)] mb-4">Vista previa pública</p>
            <div className="flex items-start gap-4">
              {profile.avatar ? <img src={profile.avatar} alt="" className="w-20 h-20 rounded-full object-cover border border-[var(--app-border)]" /> : <div className="w-20 h-20 rounded-full bg-[#0A84FF] flex items-center justify-center text-2xl font-medium">{displayName.charAt(0).toUpperCase()}</div>}
              <div className="min-w-0"><h3 className="text-xl font-medium">{displayName}</h3><p className="text-[var(--app-text-secondary)] text-sm mt-2">{profile.start_time} - {profile.end_time}</p></div>
            </div>
            {profile.bio && <p className="text-[var(--app-text-secondary)] text-sm mt-5 leading-relaxed">{profile.bio}</p>}
