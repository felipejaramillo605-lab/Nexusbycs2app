...
def invoice_pdf(invoice):
    ...
    lines=["NEXUS BY CS2","DOCUMENTO ADMINISTRATIVO DE COBRO",f"Codigo: {invoice.get('invoice_number')}",f"Emision: {str(invoice.get('issued_at',''))[:10]}   Vencimiento: {str(invoice.get('due_at',''))[:10]}","",f"PROVEEDOR: {seller.get('legal_name') or seller.get('commercial_name')}",f"NIT: {seller.get('tax_id') or 'Pendiente de configurar'}",f"Correo: {seller.get('email') or 'Pendiente de configurar'}",f"Direccion: {seller.get('address') or ''} {seller.get('city') or ''}","",f"COMPRADOR: {buyer.get('legal_name') or buyer.get('organization_name')}",f"NIT / documento: {buyer.get('tax_id') or 'Pendiente de configurar'}",f"Correo de facturacion: {invoice.get('delivery_email_snapshot') or 'No configurado'}",f"Direccion: {buyer.get('address') or ''} {buyer.get('city') or ''}","",f"Servicio: {invoice.get('service_description')}",f"Plan: {invoice.get('plan_code_snapshot')} version {invoice.get('plan_version_snapshot')}",f"Periodo facturado: {str(invoice.get('period_start'))[:10]} al {str(invoice.get('period_end'))[:10]}",f"Subtotal: {money(invoice.get('subtotal_minor'))}",f"Impuestos: {money(invoice.get('tax_minor'))}",f"TOTAL: {money(invoice.get('amount_minor'))}",f"Pagado: {money(invoice.get('paid_amount_minor'))}",f"Saldo: {money((invoice.get('amount_minor') or 0)-(invoice.get('paid_amount_minor') or 0))}",f"Estado: {invoice.get('status')}","",NOTICE]
