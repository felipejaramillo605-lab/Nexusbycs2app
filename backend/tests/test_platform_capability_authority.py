...
def test_audit_chain_detects_reordering_and_tampering():
    events = [
        {"event_id": "a", "request_id": "r1"},
        {"event_id": "b", "request_id": "r2"},
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"role": "owner", "access_status": "approved", "deleted_at": "deleted"},
            False,
        ),
...
requires_standalone = pytest.mark.skipif(
    not MONGO_URL,
    reason="set NEXUS_TEST_MONGO_URL to isolated Mongo standalone for integration tests",
)
...
def test_activation_invoice_must_be_paid_manual_premium_for_exact_org_and_request():
    ...
    for org_id, request_id, invoice_id, _label in invalid:
        with pytest.raises(HTTPException) as exc:
            _run_async(
                _validate_premium_invoice(Database(), org_id, request_id, invoice_id)
            )
        assert exc.value.status_code == 409
    ...
    assert (
        len(
            [
                ...
                if event["type"] == "bootstrap_granted"
            ]
        )
        == 1
    )
...
@requires_standalone
@pytest.mark.parametrize(
    "extra", [{"active": False}, {"deleted_at": "2026-09-22T00:00:00Z"}]
)
...
@requires_standalone
def test_bootstrap_from_environment_skips_instead_of_raising_for_a_second_owner(
    authority_db,
):
    db, sync_db = authority_db
    ...
    result = _run_async(
        bootstrap_from_environment(db, env={BOOTSTRAP_ENV_VAR: "owner-b"})
    )
...
def test_bootstrap_from_environment_skips_for_an_ineligible_owner(authority_db):
    ...
    sync_db.users.insert_one(
        {"user_id": "owner-c", "role": "manager", "access_status": "approved"}
    )
    result = _run_async(
        bootstrap_from_environment(db, env={BOOTSTRAP_ENV_VAR: "owner-c"})
    )
    assert result == {
        "status": "skipped",
        "user_id": "owner-c",
        "reason": "bootstrap target must be an approved owner",
    }
    assert sync_db.platform_capability_authority.find_one({"_id": AUTHORITY_ID}) is None
...
def test_concurrent_retries_with_same_request_id_append_one_event(authority_db):
    ...
    assert (
        sum(
            event.get("request_id") == "same-request-id"
            for event in authority["audit_events"]
        )
        == 1
    )
...
def test_concurrent_denial_events_with_same_request_id_append_once(authority_db):
    ...
    denied_actor = SimpleNamespace(
        user_id="manager-x", role="manager", access_status="approved"
    )
    ...
    results = _run_async(race())
    assert all(
        isinstance(value, HTTPException) and value.status_code == 403
        for value in results
    )
    authority = sync_db.platform_capability_authority.find_one({"_id": AUTHORITY_ID})
    assert (
        sum(
            event.get("request_id") == "denial-same-id"
            for event in authority["audit_events"]
        )
        == 1
    )
...
def test_concurrent_revokes_never_remove_last_grant(authority_db):
    ...
    _run_async(
        grant_capability(
            db, _actor("owner-a"), _owner_doc("owner-b"), "second grant", "grant-b"
        )
    )
    ...
    async def race():
        return await asyncio.gather(
            revoke_capability(
                db, _actor("owner-a"), "owner-b", "rotate grants", "revoke-b"
            ),
            revoke_capability(
                db, _actor("owner-b"), "owner-a", "rotate grants", "revoke-a"
            ),
            return_exceptions=True,
        )
...
@requires_standalone
def test_revoked_actor_cannot_start_another_mutation_and_denial_is_audited(
    authority_db,
):
    db, sync_db = authority_db
    sync_db.users.insert_many(
        [_owner_doc("owner-a"), _owner_doc("owner-b"), _owner_doc("owner-c")]
    )
    _run_async(bootstrap_initial_grant(db, "owner-a"))
    _run_async(
        grant_capability(
    ...
    with pytest.raises(HTTPException) as exc:
        _run_async(
            grant_capability(
                db, _actor("owner-a"), _owner_doc("owner-c"), "must fail", "grant-c"
            )
        )
    ...
    assert any(
        event["type"] == "denied" and event["request_id"] == "grant-c"
        for event in authority["audit_events"]
    )
...
class _DatabaseProxy:
    def __init__(self, inner, authority):
        self.inner, self.platform_capability_authority = inner, authority
...
def test_actor_revoked_after_precheck_cannot_complete_grant(authority_db):
    ...
    async def race():
        ...
        await revoke_capability(
            db, _actor("owner-b"), "owner-a", "revoke actor", "revoke-a"
        )
        barrier.resume.set()
    ...
    assert not any(
        grant["user_id"] == "owner-c" for grant in authority["active_grants"]
    )
...
def test_entitlement_can_be_set_for_legacy_org_without_explicit_false(authority_db):
    ...
    premium_request_id, invoice_id = _seed_paid_premium_invoice(
        sync_db, "legacy-org", "legacy"
    )
    result = _run_async(
        set_organization_entitlement(
            db,
...
def test_premium_invoice_activation_and_refund_share_atomic_lock(authority_db):
    ...
    async def race_activation_and_refund():
        async def activate():
            try:
                await _reserve_premium_invoice_activation(
                    db, org_id, request_id, invoice_id, "activate-race"
                )
                return True
    ...
    if activated:
        assert (
            invoice["status"] == "paid"
            and invoice["premium_activation_state"] == "reserved"
        )
        ...
        sync_db.organizations.update_one(
            {"organization_id": org_id},
            {
                "$set": {
                    ...
                    "nexus_ai_contracted": False,
                    "nexus_ai_enabled": False,
                }
            },
        )
        locked_invoice = sync_db.subscription_invoices.find_one(
            {"invoice_id": invoice_id}, {"_id": 0}
        )
        _run_async(
            _release_premium_invoice_lock(db, org_id, locked_invoice, "disable-premium")
        )
        released = sync_db.subscription_invoices.find_one({"invoice_id": invoice_id})
        assert (
            released["status"] == "paid"
            and released["premium_activation_state"] == "released"
        )
        ...
        with pytest.raises(HTTPException) as exc:
            _run_async(
                _reserve_premium_invoice_activation(
                    db, org_id, request_id, invoice_id, "activate-again"
                )
            )
        assert exc.value.status_code == 409
    else:
...
def test_disable_reactivate_disable_releases_all_premium_invoice_locks(authority_db):
    ...
    sync_db.premium_plan_requests.update_one(
        {"request_id": request_a}, {"$set": {"status": "active"}}
    )
    ...
    sync_db.subscription_invoices.insert_one(
        {
            "invoice_id": invoice_c,
            "organization_id": org_id,
            "provider": "manual",
            "invoice_purpose": "premium_plan_excess",
            "premium_request_id": request_c,
            "status": "paid",
            "amount_minor": 10000,
            "paid_amount_minor": 10000,
        }
    ...
    for invoice_id in (invoice_a, invoice_c):
        ...
        assert (
            current["status"] == "paid"
            and current["premium_activation_state"] == "released"
        )
...
def test_concurrent_entitlement_mutation_for_same_org_is_serialized(authority_db):
    ...
    _run_async(bootstrap_initial_grant(db, "owner-a"))
    sync_db.organizations.insert_one(
        {"organization_id": "org-serial", "premium_templates_contracted": False}
    )
    premium_request_id, invoice_id = _seed_paid_premium_invoice(
        sync_db, "org-serial", "serial"
    )
    ...
    invoice = sync_db.subscription_invoices.find_one({"invoice_id": invoice_id})
    premium_request = sync_db.premium_plan_requests.find_one(
        {"request_id": premium_request_id}
    )
...
def test_ledger_failure_leaves_organization_unchanged(authority_db):
    ...
    _run_async(bootstrap_initial_grant(db, "owner-a"))
    sync_db.organizations.insert_one(
        {"organization_id": "org-a", "premium_templates_contracted": False}
    )
    premium_request_id, invoice_id = _seed_paid_premium_invoice(
        sync_db, "org-a", "ledger"
    )
    ...
    assert (
        invoice["status"] == "paid"
        and invoice["premium_activation_state"] == "released"
    )
    assert (
        invoice["premium_activation_compensation_reason"]
        == "authority_event_not_persisted"
    )
...
def test_capacity_failure_compensates_reserved_invoice(monkeypatch, authority_db):
    ...
    _run_async(bootstrap_initial_grant(db, "owner-a"))
    sync_db.organizations.insert_one(
        {"organization_id": "org-capacity", "premium_templates_contracted": False}
    )
    premium_request_id, invoice_id = _seed_paid_premium_invoice(
        sync_db, "org-capacity", "capacity"
    )
    ...
    monkeypatch.setattr(
        platform_capabilities_module, "_atomic_authority_update", no_capacity
    )
    with pytest.raises(HTTPException) as exc:
        _run_async(
            set_organization_entitlement(
    ...
    assert not any(
        ...
        for event in sync_db.platform_capability_authority.find_one(
            {"_id": AUTHORITY_ID}
        ).get("audit_events", [])
    )
...
@requires_standalone
def test_revoked_capability_after_reservation_compensates_invoice(
    monkeypatch, authority_db
):
    db, sync_db = authority_db
    sync_db.users.insert_one(_owner_doc("owner-a"))
    _run_async(bootstrap_initial_grant(db, "owner-a"))
    sync_db.organizations.insert_one(
        {"organization_id": "org-revoked", "premium_templates_contracted": False}
    )
    premium_request_id, invoice_id = _seed_paid_premium_invoice(
        sync_db, "org-revoked", "revoked"
    )

    async def revoke_after_precheck(*_args, **_kwargs):
        sync_db.platform_capability_authority.update_one(
            {"_id": AUTHORITY_ID},
            {"$set": {"active_grants": []}, "$inc": {"version": 1}},
        )
        return None

    monkeypatch.setattr(
        platform_capabilities_module, "_atomic_authority_update", revoke_after_precheck
    )
    with pytest.raises(HTTPException) as exc:
        _run_async(
            set_organization_entitlement(
...
def test_pending_applied_and_unapplied_are_reconciled(authority_db):
    ...
    sync_db.organizations.insert_many(
        [
            ...
            {
                "organization_id": "org-not-applied",
                "premium_templates_contracted": False,
            },
        ]
    )
    premium_request_id, invoice_id = _seed_paid_premium_invoice(
        sync_db, "org-applied", "applied"
    )
    ...
    with pytest.raises(
        RuntimeError, match="requires an active platform maintenance lock"
    ):
    ...
    states = {
        ...
        for event in authority["audit_events"]
        if event.get("request_id") in {"ent-applied", "ent-unapplied"}
    }
    assert states == {"ent-applied": "applied", "ent-unapplied": "failed"}
    assert (
        sync_db.organizations.find_one({"organization_id": "org-applied"})[
...
@requires_standalone
def test_archived_request_id_remains_idempotent_and_cannot_be_reused(
    ...
    _run_async(bootstrap_initial_grant(db, "owner-a"))
    sync_db.organizations.insert_one(
        {"organization_id": "org-archive", "premium_templates_contracted": False}
    )
    premium_request_id, invoice_id = _seed_paid_premium_invoice(
        sync_db, "org-archive", "archive"
    )
    original = _run_async(
        set_organization_entitlement(
            db,
    ...
    tombstone = sync_db.platform_capability_request_tombstones.find_one(
        {"_id": "archive-idempotency-id"}
    )
...
@requires_standalone
def test_archive_racing_with_mutation_cannot_reuse_compacted_request_id(
    authority_db, tmp_path
):
    from archive_platform_capability_audit import archive_and_compact
    ...
    _run_async(bootstrap_initial_grant(db, "owner-a"))
    sync_db.organizations.insert_one(
        {"organization_id": "org-race", "premium_templates_contracted": False}
    )
    premium_request_id, invoice_id = _seed_paid_premium_invoice(
        sync_db, "org-race", "race"
    )
    _run_async(
        set_organization_entitlement(
            db,
    ...
    reused_event = {
        "event_id": "race-reused",
        "type": "granted",
        "request_id": "archive-race-id",
        "state": "applied",
        "created_at": "test",
    }
    ...
    authority = sync_db.platform_capability_authority.find_one({"_id": AUTHORITY_ID})
    assert not any(
        event.get("event_id") == "race-reused" for event in authority["audit_events"]
    )
    assert sync_db.platform_capability_request_tombstones.find_one(
        {"_id": "archive-race-id"}
    )
...
def test_offline_archive_file_is_hash_verified(tmp_path):
    ...
    events = [
        {"event_id": "a", "type": "granted"},
        {"event_id": "b", "type": "revoked"},
    ]
    ...
    class Collection:
        async def find_one(self, *_args, **_kwargs):
            raise AssertionError(
                "existing destination must be rejected before database access"
            )
