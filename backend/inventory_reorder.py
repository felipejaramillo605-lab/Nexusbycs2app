...
def build_inventory_reorder_router(db,get_current_user,require_management_role,resolve_team_organization):
    ...
    async def open_quantities(org,item_ids):
        pipeline=[{"$match":{"organization_id":org,"status":{"$in":sorted(OPEN_PO_STATES)},"lines.inventory_item_id":{"$in":item_ids}}},{"$unwind":"$lines"},{"$match":{"lines.inventory_item_id":{"$in":item_ids}}},{"$project":{"purchase_order_id":1,"item_id":"$lines.inventory_item_id","pending_purchase":{"$max":[{"$subtract":[{"$ifNull":["$lines.quantity",0]},{"$ifNull":["$lines.received_quantity",0]}]},0]},"factor":{"$ifNull":["$lines.conversion_factor",1]}}},{"$group":{"_id":"$item_id","open_base_quantity":{"$sum":{"$multiply":["$pending_purchase","$factor"]}},"open_order_ids":{"$addToSet":"$purchase_order_id"}}}]
        rows=await db.purchase_orders.aggregate(pipeline).to_list(100000)
        return {x["_id"]:{"open_base_quantity":round(float(x.get("open_base_quantity",0)),6),"open_order_ids":sorted(x.get("open_order_ids",[]))} for x in rows}
