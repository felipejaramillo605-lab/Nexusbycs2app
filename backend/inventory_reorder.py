...
# NEXUS_LOW_STOCK_ALERT_DAEMON_V1: module-level (not closure-bound) so the
# low_stock_alerts daemon can reuse the exact same alert computation as this
# router, instead of duplicating the aggregation/suggestion logic.
async def open_quantities(db,org,item_ids):
...
async def load_suggestions(db,org):
    ...
    opened=await open_quantities(db,org,ids);out=[]
    for item in low:
...
def build_inventory_reorder_router(db,get_current_user,require_management_role,resolve_team_organization):
    ...
    async def list_alerts(organization_id:Optional[str]=None,authorization:Optional[str]=Header(None),session_token:Optional[str]=Cookie(None)):
        _,org=await ctx(authorization,session_token,organization_id);alerts=await load_suggestions(db,org);return {"organization_id":org,"total":len(alerts),"critical_count":sum(1 for x in alerts if x["severity"]=="critical"),"warning_count":sum(1 for x in alerts if x["severity"]=="warning"),"with_supplier":sum(1 for x in alerts if x["suggested_supplier_id"]),"covered_count":sum(1 for x in alerts if x["covered_by_open_orders"]),"items":alerts}
    ...
    async def generate_orders(data:GeneratePayload,authorization:Optional[str]=Header(None),session_token:Optional[str]=Cookie(None)):
        user,org=await ctx(authorization,session_token,data.organization_id);request_key=data.idempotency_key.strip();alerts=await load_suggestions(db,org)
        if data.item_ids:
