...
def build_transaction_void_router(db,get_current_user,require_management_role,validate_organization_access):
    ...
    async def void_transaction(transaction_id:str,data:TransactionVoidRequest,authorization:Optional[str]=Header(None),session_token:Optional[str]=Cookie(None)):
        ...
        if tx.get('status')=='voided':raise HTTPException(status_code=409,detail='Transaction has already been voided')
        if tx.get('status')!='confirmed':raise HTTPException(status_code=409,detail='Only confirmed transactions can be voided')
        ...
        org=tx['organization_id'];now=datetime.now(timezone.utc).isoformat();movements=await db.inventory_movements.find({'organization_id':org,'transaction_id':transaction_id,'movement_type':'service_consumption'},{'_id':0}).to_list(1000)
        ...
        try:
            ...
            result=await db.transactions.update_one({'transaction_id':transaction_id,'status':'confirmed'},{'$set':{'status':'voided','void_reason':reason,'void_notes':(data.notes or '').strip()[:500] or None,'voided_by':user.user_id,'voided_at':now,'inventory_reversal_status':'reversed' if movements else 'not_applicable','inventory_reversal_count':len(reversal_ids)},'$unset':{'settlement_id':'','settlement_status':'','settled_at':''}})
